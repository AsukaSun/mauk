...
function convertViewTemplate(fileName){
    var fileName = path.normalize(fileName);
    var str=loadTpl(fileName);
    var pattern = /\<\!--include:([^\>\s]+.html)--\>/gi;// <!--include:file.html-->
    var result ;
    while(result = pattern.exec(str)){
        str = str.replace(result[0],convertViewTemplate (path.join( path.dirname(fileName),result[1])));
    }
    return str;
}
            ...
            if (doFn && !doFn.noAuth && !req.isAuthorized()) {
                ...
                if (doFn && (doFn.method === '*' || doFn.method === req.method)) {
                    req.waitPost && (!doFn.waitPost) && req.waitPost.call(req, function (err) {
                        ...
                        doFn.call(util._extend({app:app,req:req,res:res,next:next},doFn.modules), req, res, next);
                    });
                } else {
                    next();
                }
